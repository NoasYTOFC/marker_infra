...
class MapScreenState extends State<MapScreen> {
      ...
      children: [
        FlutterMap(
            ...
            if (_currentZoom >= 13.0)
              ...
              // Simples e compacto - Container se adapta ao tamanho do texto
              MarkerLayer(
                markers: [
                  ...provider.ctos
                      .where((cto) => _isPointInViewport(cto.posicao))
  ...
  void _showDownloadDialog(BuildContext context) {
    ...
    final tileCount = TileCacheService.calculateTileCount(
      zoomMin: 13,
      zoomMax: 17,
      latMin: bounds.south,
      latMax: bounds.north,
    ...
    final estimatedSize = TileCacheService.estimateSize(tileCount: tileCount);
        ...
        return AlertDialog(
          ...
          content: SingleChildScrollView(
            child: Column(
              ...
              children: [
                Text(
                  'Baixar tiles do OpenStreetMap da área visível para uso offline.',
                  style: TextStyle(
                    fontSize: 12,
                    color: isDarkMode ? Colors.white70 : Colors.black87,
                  ),
                ...
                Text(
                  'Tamanho estimado: ${estimatedSize.toStringAsFixed(1)} MB',
                  style: TextStyle(
                    fontWeight: FontWeight.bold,
                    color: isDarkMode ? Colors.white : Colors.black,
                  ),
                ...
                const SizedBox(height: 12),
                Text(
                  '⏱️ Isso pode levar vários minutos dependendo da conexão.',
                  style: TextStyle(
                    fontSize: 11,
                ...
                const SizedBox(height: 8),
                Text(
                  '✅ O download continua em background mesmo se você fechar o app.',
                  style: TextStyle(
                    fontSize: 11,
  ...
  void _startOfflineDownload(
    ...
    TileCacheService.onProgress = (current, total, percent) {
      print('Download: $current/$total (${(percent * 100).toStringAsFixed(1)}%)');
    };

    TileCacheService.onStatus = (message) {
      print('Status: $message');
    };

    TileCacheService.downloadOfflineMap(
      zoomMin: 13,
      zoomMax: 17,
      latMin: latMin,
      latMax: latMax,
      ...
      if (mounted) {
        ...
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(
            content: Text('✅ Mapa offline baixado com sucesso!'),
            backgroundColor: Colors.green,
...
class _DownloadProgressDialogState extends State<_DownloadProgressDialog> {
      ...
      content: SingleChildScrollView(
        child: Column(
          ...
          children: [
            ...
            // Informação
            Text(
              '⏱️ Pode levar vários minutos. O download continuará mesmo se você sair.',
              style: TextStyle(
                fontSize: 10,
  ...
  void dispose() {
    // Limpar callbacks
    TileCacheService.onStatus = null;
    TileCacheService.onProgress = null;
    super.dispose();
  }
}
